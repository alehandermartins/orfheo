  ...
  ns.Widgets.ProgramManager = function(){
      ...
      var ProposalCard = function(proposal){
        ...
        card.draggable({
          ...
          start: function(event, ui){
            _performance = {
              performance_id: Pard.Widgets.GenerateUUID(),
    ...
    var Space = function(space){
      ...
      var SpaceColumn = function(day){
        ...
        _time.droppable({
          ...
          drop: function(event, ui) {
            ...
            if(_performance.linked){ 
              performance.linked.forEach(function(linked_performance_id){
                var linked_performance = _program[linked_performance_id].performance;
                _spaces[_performance.host_id].deletePerformance(linked_performance);
                _program[linked_performance_id].destroy();
              });
            }
            else if(!_performance.host_id && day == 'permanent'){
      ...
      var AlignPerformances = function(left){
        ...
        var performances = Pard.Widgets.ReorderProgram(performances);
      ...
      return {
        ...
        addPerformance: function(performance, programCard){
          ...
          program[performance.performance_id] = performance;
          if(_daySelector.val() != performance.date) _tables[performance.date].show();
          _columns[performance.date].append(programCard);
          AlignPerformances(_columns[performance.date].position().left + 1);
          if(_daySelector.val() != performance.date) _tables[performance.date].hide();
